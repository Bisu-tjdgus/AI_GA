...
class Tour:
    ...
    def __repr__(self):
        w = open("example_solution.csv", 'w',newline='')
        writer = csv.writer(w)
        geneString = 'Start -> '
        for i in range(0, self.tourSize()):
            wlist=[]
            wlist.append(self.getCity(i))
...
class GA:                               ## mutation 확률   tornament 크기     elitism 크기
    def __init__(self, tourmanager, mutationRate=0.025, tournamentSize=8, elitism=4):
        self.tourmanager = tourmanager
    ...
    def crossover2(self, parent1, parent2, ):                           ## 11분할
        child = Tour(self.tourmanager)

        my_lst = list(random.sample(range(1, parent1.tourSize),10))
        my_lst.sort()
    ...
    def crossover3(self, parent1, parent2):                                 ## order crossover(선택 교차)
        child = Tour(self.tourmanager)
