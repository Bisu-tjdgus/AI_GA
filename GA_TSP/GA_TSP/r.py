...
class Population:
    def __init__(self, tourmanager, populationSize, initialise):
        ...
        for i in range(0, populationSize):
            self.sorted_tours.append(None)

        if initialise:
            for i in range(0, populationSize):
                newTour = Tour(tourmanager)
                newTour.generateIndividual()
                self.saveSorted_Tour(i, newTour)
    ...
    def __setitem__(self, key, value):
        self.sorted_tours[key] = value

    def __getitem__(self, index):
        return self.sorted_tours[index]

    def saveTour(self, index, tour):
        self.tours[index] = tour

    def saveSorted_Tour(self,index, tour):
        self.sorted_tours[index] = tour

    def getTour(self, index):
        return self.tours[index]

    def getSortTour(self, index):
        return self.sorted_tours[index]
    '''
    def getFittest(self):
        fittest = self.tours[0]
        for i in range(0, self.populationSize()):
            print(i,"번째 숫자", self.getTour(i).getFitness())
            if fittest.getFitness() <= self.getTour(i).getFitness():
                fittest = self.getTour(i)
        return fittest'''

    def getFittest(self):
        return self.getSortTour(0)
    ...
    def SortFittest(self):
        sorted_Fitness=[]
        for i in range(0, self.populationSize()):
            sorted_Fitness.append(self.getTour(i).getFitness())
        sorted_Fitness.sort(reverse=True)
        for i in range(0, self.populationSize()):
            self.saveSorted_Tour(i,sorted_Fitness[i])
...
class GA:
    def __init__(self, tourmanager, mutationRate=0.05, tournamentSize=25, elitism=5):
        self.tourmanager = tourmanager
    ...
    def evolvePopulation(self, pop):
        ...
        if(self.elitism>0):
            ...
            for i in range(0,self.elitism):
                newPopulation.saveTour(i, pop.getSortTour(i))
                elitismOffset = elitismOffset+1
...
if __name__ == '__main__':
    ...
    for i in range(n_generations):
        ...
        cv2.putText(map_result, org=(10, 950), text='Generation: %d' % (i + 1), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
        ...
        cv2.putText(map_result, org=(900, 950), text='Group 3',
